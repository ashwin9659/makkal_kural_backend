...
const createComplaint = asyncHandler(async (req, res) => {
  ...
  const isOtherCategory = ["other", "others"].includes(
    categoryDoc.name.trim().toLowerCase(),
  );
  if (!isOtherCategory) {
    const subCategoryDoc = await SubCategory.findById(subCategory);
    if (
      !subCategoryDoc ||
      String(subCategoryDoc.category) !== String(category)
    ) {
      res.status(400);
  ...
  if (!req.user.ward) {
    res.status(400);
    throw new Error("Your account has no ward set - please contact support");
  }
...
const getMyComplaints = asyncHandler(async (req, res) => {
  ...
  const data = complaints.map((complaint) => {
    ...
    complaintData.adminPhoto = complaintData.adminPhotos?.[0] || null;
...
const getComplaintStats = asyncHandler(async (req, res) => {
    ...
    Complaint.aggregate([
        ...
        : [{ $match: { ward: req.user.ward } }]),
      {
        $group: {
          ...
          totalResolved: {
            $sum: {
              $cond: [{ $in: ["$status", ["Completed", "Rejected"]] }, 1, 0],
            },
          },
          totalPending: {
  ...
  const wardStatsById = new Map(
    wardComplaintStats.map((item) => [String(item._id), item]),
  );
  ...
  stats.wardSnapshot =
    req.user.role === "admin" ? wardSnapshots : wardSnapshots[0] || null;
...
const updateComplaintStatus = asyncHandler(async (req, res) => {
  const { status, adminRemarks, adminPhoto, adminPhotos } = req.body;

  if (
    !["Pending", "Received", "In Progress", "Completed", "Rejected"].includes(
      status,
    )
  ) {
    res.status(400);
    throw new Error(
      "Status must be one of Pending, Received, In Progress, Completed, Rejected",
    );
  }
  ...
  if (Array.isArray(adminPhotos)) complaint.adminPhotos = adminPhotos;
  if (typeof adminPhoto === "string") complaint.adminPhotos = [adminPhoto];
